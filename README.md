...
- **Команды:** `-inf`, `-cal` (40 регистров: чтение и запись, хранятся в
  ...
  Заводские команды настройки (`-ris`, `-fall`, `-U`, `-V`, `-nos`,
  `-pileup`…) не принимаются: параметры обработки задаются на странице
  прибора.
- **Во время набора** каждую секунду — пакет статуса (время набора,
  загрузка, CPS, отброшенные импульсы), затем весь спектр: 8192 канала
  пакетами по 64. `-sta` запускает набор спектра и на самом приборе,
  `-sto` — останавливает.
- В ответе `-inf` `RISE` — окно трапеции L, `FALL` — перезапуск после
  пика: по `(RISE+FALL+1)/F` программы считают мёртвое время на импульс.
...
формат `-inf`/`-cal`, порядок статуса и спектра).
