...
esp_err_t adc_cap_start(void)
{
    ...
    /* НОВЫЙ ЗАПУСК - РАЗРЫВ ПОТОКА. Между остановкой и запуском данных
     * нет, а буферы захвата DMA пишет заново с первого. Без этого фильтр
     * спектра считал трапецию через стык старых и новых отсчётов (на
     * стыке могло родиться ложное событие), а осциллограф брал
     * предысторию из буферов, которые уже перезаписываются. Признак
     * ставим ДО запуска DMA, чтобы его увидел уже первый новый чанк:
     * задача обработки сбросит и фильтр, и предысторию. Захват теперь
    ...
    if (s_chunks == before) {
        ...
        return ESP_ERR_TIMEOUT;      /* захват оставляем включённым,
                                        чтобы диагностика на странице могла наблюдать */
    }
